...
class NgoDistribution(models.Model):
    ...
    distribution_schedule_id= fields.Many2one('ngo.distribution.schedule',string=_(u"Distribution Schedule"),required=True,)
    ...
    distribution_kind = fields.Selection([('Ibad', 'العباد'),
                                      ('Association', 'جهة')], string=_(u"Distribution Kind"), required=1,default="Ibad")
    ...
    def action_generate_barcodes(self):
        ...
        for app in distribution_lines:
            if not app.order_code:
                ...
                while not app.order_code  and  self.env['ngo.distribution.line'].search_count([('order_code','=',barcode),('distribution_id.id','=',self.id)]) >0:
                    barcode = self.generate_12_random_numbers() +  self.code[2:len(self.code)]
    ...
    def action_distribute_applications_perdays(self):
        listtodistribute = self.env['ngo.distribution.line'].search([('distribution_id','=',self.id)], order='application_type_id asc')
        ...
        if self.distribution_schedule_id:
            ...
            for application in listtodistribute:
                if cnt < self.expected_distribution_perday:
                    ...
                    while bisect.bisect_left(days, start_time_day) == False: # str(days.index(start_time_day)) not in ('0','1','2','3','4','5','6'):
                    ...
                    if (timedelta(hours = delivery_time) -   timedelta(hours = hourstoadd))   > (timedelta(hours = delivery_time2)):
                        application.delivery_time = timedelta(hours = delivery_time)  + timedelta(hours = delivery_time2) -  timedelta(hours = delivery_time) +  timedelta(hours = hourstoadd)
                    else:
...
class NgoDistributionLine(models.Model):
    ...
    partner_id = fields.Many2one('res.partner',string=_(u"Related Partner"),domain=[('is_application','=',True)])# ADD DOMAIN
    ...
    def _compute_application_info(self):
        for application in self:
          if application.application_id:
            application_rec = application.env['ngo.beneficiary.application'].search([('id', '=', application.application_id.id)], limit=1)
            if application_rec:
    ...
    def get_delivery_date_text(self):
        for record in self:        
            if record.delivery_date:
                ...
                delivery_date_text = self.dayofweekarabic(dayoftheweekreceiptdate) + " في " + str(dayreceiptdate) + " " + self.montharabic(monthreceiptdate) + " " + str(yearreceiptdate) +" الساعة " +str(timereceiptdate)
