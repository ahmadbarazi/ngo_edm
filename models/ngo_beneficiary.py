...
class Beneficiary(models.Model):
    ...
    responsible_id = fields.Many2one('ngo.beneficiary',string=_(u"Responsible"),domain=[('is_responsible','=',True)])
    ...
    medical_record_ids = fields.One2many('ngo.beneficiary.medical.record','beneficiary_id',string=_(u"Medical Record"))
    medical_visit_ids = fields.One2many('ngo.beneficiary.medical.visit','beneficiary_id',string=_(u"Medical Record"))
    education_record_ids = fields.One2many('ngo.beneficiary.education.record','beneficiary_id',string=_(u"Education Record"))
    ...
    ##### END MEMBER DETAILS #####
    identity_no = fields.Char(string=_("Identity Number"))
    ...
    def write(self, vals):
        ...
        for beneficiary in self:
            ...
            if beneficiary.name!=(str(beneficiary.first_name or '') + ' ' +str(beneficiary.father_name or '')+ ' ' +str(beneficiary.last_name or '')):
                beneficiary.name = (str(beneficiary.first_name or '') + ' ' +str(beneficiary.father_name or '')+ ' ' +str(beneficiary.last_name or ''))
        return res
...
class BeneficiaryIncome(models.Model):
    ...
    income_amount = fields.Monetary(string=_("Income Amount"))
    ...
    _sql_constraints = [
            ('date_check', "CHECK ((start_date <= stop_date))", "The stop date must be greater than the start date in income info.")
        ]    
...
class BeneficiaryMedicalRecord(models.Model):
    ...
    _sql_constraints = [
            ('date_check', "CHECK ( (start_date <= stop_date))", "The stop date must be greater than the start date in medical record info.")
        ]
...
class BeneficiaryEmploymentHistory(models.Model):
    ...
    _sql_constraints = [
            ('date_check', "CHECK ( (start_date <= stop_date))", "The stop date must be greater than the start date in employment history info.")
        ]